...
public class KClosestPointsToOrigin {
    ...
    static ArrayList<ArrayList<Integer>> kClosestPointsToOrigin(ArrayList<ArrayList<Integer>> points, int k){
        /*
        PriorityQueue<ArrayList<Integer>> queue = new PriorityQueue<>(Comparator.comparingInt(a -> (Math.abs(a.get(0)) + Math.abs(a.get(1)))));
        for(ArrayList<Integer> point: points) {
            if (queue.size() < k) queue.add(point);
            else {
                int D = Math.abs(queue.peek().get(0)) + Math.abs(queue.peek().get(1));
                int A = Math.abs(point.get(0)) + Math.abs(point.get(1));
                if (D < A) {
                    queue.poll();
                    queue.add(point);
                }
